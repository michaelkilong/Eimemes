...
export default function AdminGalleryPage() {
  ...
  const inputClass =
    'w-full bg-[#0d0f14] border border-[#2a2f3d] rounded-sm px-4 py-2.5 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-[#d97706] transition-all font-mono';
  const labelClass =
    'block text-[10px] font-mono uppercase tracking-widest text-slate-500 mb-1.5';
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Match action photo..."
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Player celebrating goal..."
                    />
                  </div>
                  <div>
                    ...
                    <button
                      ...
                      className="w-full btn-ghost border-[#2a2f3d] text-slate-400 text-xs py-2.5 mb-2"
                      disabled={uploading}
                    >
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {items.map(item => (
                <div
                  key={item._id}
                  className="group relative bg-[#13171f] border border-[#1e2433] rounded-sm overflow-hidden"
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                    ...
                    <button
                      ...
                      className="p-2 bg-red-500/20 border border-red-500/30 rounded-sm text-red-400 hover:bg-red-500/30 transition-colors"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
                  <div className="p-3">
                    ...
                    {item.caption && (
                      <p className="text-[10px] text-slate-600 truncate mt-0.5">{item.caption}</p>
                    )}
                    <div className="flex items-center gap-2 mt-1 text-[10px] font-mono text-slate-600">
                      <span>{item.category}</span>
                      {item.branch && (
                        <span className="text-[#d97706] border border-[#d97706]/30 rounded-sm px-1.5 py-0.5">
                          {item.branch}
                        </span>
                      )}
                    </div>
